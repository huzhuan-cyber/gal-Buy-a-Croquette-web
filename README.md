...
原版根目录的 `index.html` 是启动/说明页；用 `web/index.html` 这个**增强版加载器**替换（复制到游戏根目录作为 `index.html`）。它包含：

- **引擎脚本加载器**：按依赖顺序串行加载 RPG Maker MV 引擎 + 进度条
- **音频解锁**：首次点击「开始游戏」才解锁音频（绕过浏览器自动播放策略）
- **流量警告**：游戏约 350MB，检测到移动网络时弹窗确认（Safari 用 effectiveType 兜底）
- **存档管理**：导出 / 导入存档（防换设备丢档）
- **iOS 适配**：横屏提示、内联视频、离线缓存限制提示
- **PWA 集成**：Service Worker 注册、版本更新提示、离线状态指示
- **后台预缓存**：进入游戏后自动预缓存 `data/` 数据文件，二次进入明显变快
...
- **音频不播放**：浏览器自动播放策略导致，点入口的「开始游戏」按钮即可解锁（加载器已处理）
- **Steam 成就不可用**：网页版没有 Steam 环境，`MK_SimpleGreenworks` 会自动跳过成就逻辑，不影响游玩
- **存档位置**：RPG Maker MV 网页版存档保存在浏览器本地存储中，**同一浏览器同一设备**内有效；清除浏览器数据会丢档。可用引导页的「导出存档 / 导入存档」备份迁移
- **iOS 离线功能不完整**：Safari 对 Service Worker 缓存配额有限，弱网 / 离线时建议在 WiFi 环境游玩
- **字体显示异常**：确认 `fonts/` 目录完整，入口页引用了 `fonts/gamefont.css`
